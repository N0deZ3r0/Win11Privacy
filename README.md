...
| **Проба связи** | Кнопка на странице «Проверка»: компьютер сам звонит по двенадцати адресам сбора данных и показывает, кто ещё отвечает. Не «настройка на месте», а состояние канала. Соединение открывается и тут же закрывается — данные не отправляются. |
