...
| | |
|---|---|
| **Досье** | Кто и когда включал вашу камеру, микрофон и геолокацию — с длительностью и меткой «сейчас». Плюс цифровой след на диске: рекламный ID, история сетей Wi-Fi и флешек, история буфера обмена, история команд PowerShell, RDP-подключения, кэш эскизов, снимки экрана Recall — с выборочным стиранием. |
...
| **Монитор утечек** | Кто и куда реально отправляет данные — по журналу брандмауэра. |
...
| **Проба связи** | Кнопка на странице «Проверка»: компьютер сам звонит по одиннадцати адресам сбора данных — тем самым, которые программа умеет закрывать, — и показывает, кто ещё отвечает. Не «настройка на месте», а состояние канала. Соединение открывается и тут же закрывается — данные не отправляются. |
| **Имя компьютера в эфир** | Windows объявляет себя в локальной сети (LLMNR, mDNS, NetBIOS) и регулярно звонит в Microsoft «проверить интернет». По этим запросам компьютер видно и в кафе, и в офисе — программа их выключает. |
| **Истории, которые копятся заново** | «Досье» стирает накопленное, а отдельный модуль запрещает копить дальше: недавние документы, история поиска и Win+R, фоновая запись игр, почта на экране входа. |

Ещё: телеметрия сторонних программ (Chrome, Edge, Office, VS Code, NVIDIA, PowerShell, Visual Studio),
слежка производителя ноутбука, синхронизация настроек и паролей с учётной записью, связь с телефоном,
блокировка через брандмауэр, стирание накопленного буфера телеметрии,
...
Интерфейс полностью на русском и английском, включая названия всех 226 настроек.
...
| **MainForm.cs** | Окно, девять страниц, запуск движка, разбор его ответов, отчёт HTML, режим командной строки. |
...
| **Lang.cs** | Английский перевод интерфейса (собирается из lang-en*.txt). |
...
| `-Probe` | проба связи: дозвониться до адресов сбора данных и вернуть, кто ответил |
...
типы: `reg`, `regif` (только если ветка уже есть), `svc`, `svcopt`, `task`,
`taskglob`, `env`, `vscode`, `firefox`, `hosts`, `fwsvc`, `fwapp`, `fwips`.
...
- на каждый коммит в main и каждый pull request — проверяется синтаксис движка,
  собирается exe и проверяется, что движок вшит внутрь и совпадает с исходником,
  а манифест запрашивает права администратора;
