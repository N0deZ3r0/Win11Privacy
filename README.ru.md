...
1. Обновите номер версии в `app.res` и в заголовке программы.
2. Закоммитьте изменения в `main` — автосборка проверит движок, перевод и интерфейс.
...
4. По тегу вида `v*` сборка сама создаст релиз и приложит к нему
   `Win11Privacy.exe`, `Win11-Privacy-Engine.ps1` и `SHA256SUMS.txt`.
