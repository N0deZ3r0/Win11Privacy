...
echo [1/7] Opredelenie sistemy...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ENG%" -Detect      > "%~dp0result-detect.json"   2>&1

echo [2/7] Proverka nastroek...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ENG%" -Audit       > "%~dp0result-audit.json"    2>&1

echo [3/7] Status rentgena telemetrii...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ENG%" -XrayStatus  > "%~dp0result-xray.json"     2>&1

echo [4/7] Dosye: dostup k kamere/mikrofonu/geolokacii...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ENG%" -Spy         > "%~dp0result-spy.json"      2>&1

echo [5/7] Dosye: cifrovoy sled na diske...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ENG%" -Footprint   > "%~dp0result-footprint.json" 2>&1

echo [6/7] Proba svyazi s adresami sbora dannyh...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ENG%" -Probe       > "%~dp0result-probe.txt"     2>&1

echo [7/7] Testovyy progon primeneniya (nichego ne menyaet)...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ENG%" -DryRun -NoBackup -NoRestorePoint -Modules telemetry,ads,copilot,ai,location,widgets,network,sync,history,cleanup,startup > "%~dp0result-dryrun.txt" 2>&1

echo.
echo Gotovo. Ryadom sozdany 7 faylov s rezultatami:
echo   result-detect.json
echo   result-audit.json
echo   result-xray.json
echo   result-spy.json
echo   result-footprint.json
echo   result-probe.txt
