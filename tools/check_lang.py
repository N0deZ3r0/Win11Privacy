...
ENGINE_LIT = [
    ...
    # название настройки — последний строковый аргумент Def (за ним может
    # стоять только минимальная сборка Windows)
    re.compile(r"^Def\s+'[^']+'\s+'[^']+'.*'([^']*[А-Яа-яЁё][^']*)'(?:\s+\d+)?\s*$", re.M),
    # пункты «Цифрового следа»: & $add 'id' 'название' 'описание'
    re.compile(r"&\s*\$add\s+'[^']+'\s+'([^']+)'"),
    re.compile(r"&\s*\$add\s+'[^']+'\s+'[^']+'\s+'([^']+)'"),
]
